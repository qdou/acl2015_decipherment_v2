...
In figure~\ref{fig:overlap}, we see an instance of a recurring phenomenon, where translation pairs are very close and sometimes even overlap each other, for example (judge, jueces), (secret, secretos). The word ``magistrado'', which has been placed close to its possible translations, does not appear in our evaluation set. Thus, our approach is capable of learning word translations that cannot be discovered from limited parallel data. We often also see \emph{translation clusters}, where semantic groups of words are close to each other. For example, in figure~\ref{fig:trans-cluster}, we can see that time expressions in Spanish are quite close to their translations in English. Although better quality translation visualizations~\cite{mikolov2013exploiting} have been presented in previous work, they exploit large amounts of parallel data to learn the mapping between source and target words, while our transformation is learned on \emph{non-parallel} data. 
...
\end{figure}
 ...
 \begin{figure}[!ht]
  ...
  \caption{We also see semantic groups of word-translations close to each other.}
