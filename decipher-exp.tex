...
Since our learned transformation matrix $M$ significantly improves decipherment accuracy, it's likely that it is \emph{translation preserving}, that is, plaintext words are transformed from their native vector space to points in the ciphertext such that translations are close to each other. To visualize this effect, we take the top $5000$ plaintext words and transform them into embeddings $v_{\plain'} = v_{\plain}^T M$. We then project the top $5000$ ciphertext words and the projected plaintext words into a $2-$dimensional space using t-sne~\cite{van2008visualizing}. In figure~\ref{fig:viz-1}, we see an instance of a recurring phenomenon, where translation pairs are very close and sometimes even overlap each other. We often also see \emph{translation clusters}, where semantic groups of words are close to each other. We can see that time expressions in Spanish are quite close to their translations in English. Although high quality translation visualization have been presented in previous work, most, if not all have been learned using parallel data, unlike our setting where our transformation is learned on \emph{non-parallel} data. 
